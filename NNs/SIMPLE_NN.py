...
class SIMPLE_NN(nn.Module):
    ...
    Args:
        ...
        defvals: list
            list of floats corresponding to defocus values of the input images. This is
            not used for training, but stored for record keeping.
        nb_filters: int
    ...
    def __init__(
        ...
        self.upsample_block4a = upsample_block(
            nb_filters * 8, nb_filters * 4, mode=upsampling_mode
        )
        ...
        self.upsample_block3a = upsample_block(
            nb_filters * 4, nb_filters * 2, mode=upsampling_mode
        )
        ...
        self.upsample_block2a = upsample_block(
            nb_filters * 2, nb_filters, mode=upsampling_mode
        )

        self.cu2p = conv2dblock(1, nb_filters, nb_filters, use_batchnorm=batch_norm)
        self.cu2a = conv2dblock(1, nb_filters, nb_filters, use_batchnorm=batch_norm)

        self.upsample_block1p = upsample_block(nb_filters, 1, mode=upsampling_mode)
        self.upsample_block1a = upsample_block(nb_filters, 1, mode=upsampling_mode)
    ...
    def forward(self, x):
        ...
        # # amp
        # if self.learn_amp:
        #     # amp up # need if learn_amp at some point
        #     u4_a = self.cu4a(self.upsample_block4a(bn))
        #     u3_a = self.cu3a(self.upsample_block3a(u4_a))
        #     u2_a = self.cu2a(self.upsample_block2a(u3_a))
        #     amp = self.upsample_block1a(u2_a)
        # else:
        #     amp = torch.empty(0)
